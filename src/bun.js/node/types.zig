...
pub const Path = struct {
    ...
    inline fn getPathBufLenT(comptime T: type) usize {
        return if (@TypeOf(T) == u16) MAX_PATH_BYTES / 2 else MAX_PATH_BYTES;
    }

    inline fn getBufAllocT(comptime T: type, allocator: std.mem.Allocator, len: usize) ![]T {
        const tmp = getPathBufLenT(T);
        const varedLen = if (len > tmp) len else tmp;
        return try allocator.alloc(T, varedLen);
    }
    ...
    inline fn posixCwd(buf: []u8) MaybeBuf(u8) {
        const cwd = switch (getCwd(buf)) {
            .result => |r| r,
            .err => |e| return MaybeBuf(u8){ .err = e },
        };
        const len = cwd.len;
        if (len == 0) {
            return MaybeBuf(u8){ .result = cwd };
        }
        if (comptime Environment.isWindows) {
            ...
            if (index) |_index| {
                return MaybeBuf(u8) { .result = normalizedCwd[_index ..len] };
            }
            return MaybeBuf(u8) { .result = normalizedCwd[len - 1 ..len] };
        }
    ...
    pub inline fn getCwd(buf: []u8) MaybeBuf(u8) {
        const cwd = bun.getcwd(buf) catch {
            return MaybeBuf(u8).errnoSys(0, Syscall.Tag.getcwd).?;
        };
        return MaybeBuf(u8){ .result = cwd[0 ..cwd.len] };
    }
    ...
    pub fn basename(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        var path = path_ptr.toSlice(globalObject, allocator);
        defer path.deinit();

        if (suffix_ptr) |_suffix_ptr| {
            var suffix = _suffix_ptr.toSlice(globalObject, allocator);
            defer suffix.deinit();
            const out =
                if (!isWindows)
                    basenamePosix(path.slice(), suffix.slice())
                else
                    basenameWindows(path.slice(), suffix.slice());
            return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
        }

        const out =
            if (!isWindows)
                basenamePosix(path.slice(), null)
            else
                basenameWindows(path.slice(), null);
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn dirname(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const out = if (!isWindows)
            dirnamePosix(path.slice())
        else
            dirnameWindows(path.slice());
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn extname(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const out = if (!isWindows)
            extnamePosix(path.slice())
        else
            extnameWindows(path.slice());
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn format(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const buf = getBufAllocT(u8, allocator, lenSum) catch bun.outOfMemory();
        defer allocator.free(buf);

        const pathObject = .{ .root = root, .dir = dir, .base = base, .ext = ext, .name = _name };
        const out = if (!isWindows) 
            formatPosix(pathObject, buf) 
        else 
            formatWindow(pathObject, buf);
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn isAbsolute(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        var pathZigStr = path_ptr.getZigString(globalObject);
        if (!isWindows) {
            var stack_fallback = std.heap.stackFallback(
                stack_fallback_size_small, 
                JSC.getAllocator(globalObject)
            ...
            defer path.deinit();
            return JSC.JSValue.jsBoolean(isAbsolutePosix(path.slice()));
        }
        return JSC.JSValue.jsBoolean(isAbsoluteWindowsZigString(pathZigStr));
    }
    ...
    pub fn join(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        if (comptime is_bindgen) return JSC.JSValue.jsUndefined();
        if (args_len == 0) return JSC.ZigString.initUTF8(CHAR_STR_DOT).toValueGC(globalObject);
        ...
        for (0 ..args_len) |i| {
            ...
            const path = path_ptr.toSlice(globalObject, allocator).slice();
            const len = path.len;
            paths[i] = path;
        ...
        const varLenU8Buf1 = getBufAllocT(u8, allocator, lenSum) catch bun.outOfMemory();
        defer allocator.free(varLenU8Buf1);

        const out = if (!isWindows) 
            joinPosix(paths, buf, varLenU8Buf1)
        else 
            joinWindows(paths, buf, varLenU8Buf1);
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn normalize(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const len = path.len;
        if (len == 0) return JSC.ZigString.initUTF8(CHAR_STR_DOT).toValueGC(globalObject);

        const buf = getBufAllocT(u8, allocator, len) catch bun.outOfMemory();
        defer allocator.free(buf);

        const out = if (!isWindows)
            normalizePosix(path.slice(), buf)
        else
            normalizeWindows(path.slice(), buf);
        return JSC.ZigString.init(out).withEncoding().toValueGC(globalObject);
    }
    ...
    pub fn parse(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const out = if (!isWindows)
            parsePosix(path.slice())
        else
            parseWindows(path.slice());

        var jsObject = JSC.JSValue.createEmptyObject(globalObject, 5);
        jsObject.put(globalObject, JSC.ZigString.static("root"), JSC.ZigString.init(out.root).withEncoding().toValueGC(globalObject));
        jsObject.put(globalObject, JSC.ZigString.static("dir"), JSC.ZigString.init(out.dir).withEncoding().toValueGC(globalObject));
        jsObject.put(globalObject, JSC.ZigString.static("base"), JSC.ZigString.init(out.base).withEncoding().toValueGC(globalObject));
        jsObject.put(globalObject, JSC.ZigString.static("ext"), JSC.ZigString.init(out.ext).withEncoding().toValueGC(globalObject));
        jsObject.put(globalObject, JSC.ZigString.static("name"), JSC.ZigString.init(out.name).withEncoding().toValueGC(globalObject));
        return jsObject;
    }
    ...
    pub fn relative(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        var from = from_ptr.toSlice(globalObject, allocator);
        defer from.deinit();

        var to = to_ptr.toSlice(globalObject, allocator);
        defer from.deinit();

        const lenSum = from.len + to.len;

        const buf = getBufAllocT(u8, allocator, lenSum) catch bun.outOfMemory();
        defer allocator.free(buf);

        const varedLenBuf = getBufAllocT(u8, allocator, lenSum) catch bun.outOfMemory();
        defer allocator.free(varedLenBuf);

        const varedLenBuf2 = getBufAllocT(u8, allocator, lenSum) catch bun.outOfMemory();
        defer allocator.free(varedLenBuf2);

        const maybe = if (!isWindows)
            relativePosix(from.slice(), to.slice(), buf, varedLenBuf, varedLenBuf2)
        else
            relativeWindows(from.slice(), to.slice(), buf, varedLenBuf, varedLenBuf2);

        return switch (maybe) {
            .result => |r| JSC.ZigString.init(r).withEncoding().toValueGC(globalObject),
            .err => |e| e.toJSC(globalObject),
        };
    }
    ...
    pub fn resolvePosixT(comptime T: type, paths: []const []const T, buf: []T, varedLenBuf: []T) MaybeSlice(T) {
        ...
        while (i_i64 > -2 and !resolvedAbsolute) : (i_i64 -= 1) {
            ...
            if (i_i64 >= 0) {
                ...
                var tmpU8Buf: [getPathBufLenT(T)]T = undefined;
                path = switch (posixCwd(&tmpU8Buf)) {
                    .result => |r| brk: {
                        if (@TypeOf(T) == u16) {
                            // Reuse varedLenBuf because it's used for path.
                            bufSize = std.unicode.utf8ToUtf16Le(varedLenBuf, r) catch {
    ...
    pub fn toNamespacedPath(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const varedLenBuf = getBufAllocT(u8, allocator, len) catch bun.outOfMemory();
        defer allocator.free(varedLenBuf);

        return switch (toNamespacedPathWindows(path.slice(), buf, varedLenBuf)) {
            .result => |r| JSC.ZigString.init(r).withEncoding().toValueGC(globalObject),
            .err => |e| e.toJSC(globalObject),
        };
    }
...
pub const Process = struct {
    ...
    pub fn getCwd(globalObject: *JSC.JSGlobalObject) callconv(.C) JSC.JSValue {
        ...
        var buffer = brk: {
            const top_level_dir_buf = fs.top_level_dir_buf;
            if (top_level_dir_buf.len > 0) {
                break :brk top_level_dir_buf;
            }
            const u8Buf: PathBuffer = undefined;
            break :brk u8Buf;
        };
        return switch (Path.getCwd(&buffer)) {
            .result => |r| JSC.ZigString.init(r).withEncoding().toValueGC(globalObject),
            .err => |e| e.toJSC(globalObject),
        };
    }
